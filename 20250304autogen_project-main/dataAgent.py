...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    prompt = (                                    #這段要自己修正
        ...
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供學生成績建議。"
        "其中請特別注意：\n"
        "  1. 分析學生的成績分布；\n"
        "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出成績低落學生的教學建議資訊，\n"
        "     並將搜尋結果整合進回覆中；\n"
        "  3. 最後請提供具體的建議和相關參考資訊。\n"
...
async def main():    #主程式
    ...
    csv_file_path = "807.csv"  #可改成讀取自己的相關資料
    chunk_size = 1000
    chunks = list(pd.read_csv(csv_file_path, chunksize=chunk_size, encoding="utf-8-sig"))
    ...
    output_file = "807成績建議.csv"  #目標檔名
    df_log.to_csv(output_file, index=False, encoding="utf-8-sig")    #儲存結果
